...
## Phase 2 — TTSサーバープロキシ（最初のCloud Function）✅ サーバー側コード実装済み（2026-07-15）
**目的**：ElevenLabsキーの平文露出を無くす。他フェーズと独立して着手・出荷できる、最優先の実害対策。
**前提**：Firebaseプロジェクトを無料(Spark)から従量課金(Blaze)プランへ切替が必要
（Cloud FunctionsからElevenLabsへの外部通信にBlazeが必須）。**この切替はユーザー側の作業**。

### 実装内容（コミット済み）
- `functions/lib/tts.js`：純粋ロジック（家族コード/キー形式のバリデーション、1日あたり文字数
  クォータの判定・消費、ElevenLabsのHTTPステータス→クライアント向けエラーの整形）。
  `firebase-functions`/`firebase-admin`をimportしない設計＝依存ゼロでNodeテスト可能
  （`tests/unit-tts-functions.js`・41項目）。
- `functions/index.js`：onRequest（HTTPS）関数3つの薄いグルー。**callable関数ではなくonRequestを
  選んだ理由**：`synthesizeSpeech`が音声バイナリを返す必要があり、callableのJSON包装
  （base64化で+33%）より素の`fetch`+`Content-Type:audio/mpeg`の方が自然で、クライアントに
  `firebase-functions-compat.js`を追加で読み込ませずに済むため。
  - `setTtsKey`：キーをElevenLabsへ検証してから `families/{code}/private/tts` に保存
  - `verifyTtsKey`：保存済みキーの有効性確認（設定画面の「接続確認」用）
  - `synthesizeSpeech`：保存済みキーで音声合成し、音声バイナリをそのまま返す。
    「声が見つからない」時はクライアントが渡す`ownerId`でボイスライブラリに追加後1回だけ再試行
    （既存の`fetchVoiceBlob`+`addSharedVoice`の挙動を踏襲）
  - 認証：`Authorization: Bearer <Firebase IDトークン>`を`admin.auth().verifyIdToken()`で検証
    （匿名認証のトークンでも可＝現状のFirestoreルールと同じ認証レベルを維持。家族コードの
    所有検証はまだ無い＝Phase 4まで残る既知の制約）
  - CORSは`https://km4145-ship-it.github.io`＋ローカル開発サーバーのみに限定
  - リージョン：`asia-northeast1`（東京）
- `firestore.rules`：`families/{fam}/private/{document=**}` を `allow read, write: if false` で
  完全ロックダウン（既存の`match /{document=**}`より具体的なパスとして優先される）。
- **防御的な日次クォータ**：1家族あたり1日50,000文字（`DAILY_CHAR_LIMIT`）。家族コードを
  推測した第三者がプロキシ経由でその家族の鍵を乱用しても被害を上限で抑える
  （鍵そのものは守れても「使われる」余地はPhase 4の実アカウントモデルまで残るため）。
- `cloud-sync.js`の`cloudWipeAll`（完全リセット）に`private`サブコレクションの削除を追加
  （見落としていると、完全リセットしてもサーバー上に鍵が残り続けるバグになるため）。

### まだ実行していないこと（ユーザー側の作業として引き継ぎ）
1. **Firebaseプロジェクトの Spark → Blaze プラン切替**（Firebaseコンソール → 使用量と請求）
2. ローカル動作確認（推奨・任意）：`cd functions && npm install` の後、
   `firebase emulators:start --only functions,firestore` でエミュレータを起動し、
   ```
   curl -X POST http://localhost:5001/study-app-48c8f/asia-northeast1/verifyTtsKey \
     -H "Content-Type: application/json" -d '{"familyCode":"0000"}'
   ```
   （認証ヘッダ無しなら401が返ることを確認するだけでも配線ミスの早期発見になる）
3. **デプロイ**：`firebase deploy --only firestore:rules,functions`
   （ルールも一緒に反映。functionsだけ先に配ってprivateパスが無防備なままにしない）
4. デプロイ後、実際にElevenLabsのAPIキーで動作確認（設定画面からキー入力→接続確認→読み上げ）
5. 4が確認できたら、**クライアント切替コミット**（`cloud-sync.js`/`index.html`を新エンドポイント
   呼び出しに変更する別コミット。同じセッションで用意済み）をpushする。
   **この順序が重要**：関数のデプロイより先にクライアント切替をpushすると、家族のTTSが
   その場で壊れる（関数が存在しないURLを叩くことになるため）。

**要判断（前回セッションで決定済み）**：カスタムバックエンド vs Firebase-only。
**Firebase Cloud Functionsを採用**（すでに全面Firebase構成・Secret Manager統合・
ゼロスケール・新規ホスティング不要）。このデプロイパターンはPhase 4（招待コード）・
Phase 6（課金Webhook）でも再利用する。
