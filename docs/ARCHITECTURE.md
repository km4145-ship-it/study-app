...
| 12 | `js/reading-ja.js` | 関数＋データ | 日本語の読み変換(TTS誤読対策)。hasJapanese/intToKan/numToJa/toReading/forTTS 等。自己完結 | なし |
| 13 | `js/content-data.js` | 純データ | 学習コンテンツ（FLASH_DECKS/STUDY_NOTES/WRITE_PROBLEMS） | なし |
| 14 | `js/three.min.js` | ベンダー | Three.js r185（esbuild で IIFE 化・グローバル `THREE`。再生成は §「3Dキャラ」参照） | なし |
| 15 | `js/char3d.js` | 関数＋純データ | 3Dキャラ（CHAR3D_SPECS 12体・char3dBuild/char3dTag/char3dHydrate/char3dObserve） | **14 の後**（THREE を使う。load時は触らない） |
| 16 | `index.html` inline `<script>` | 結合コア | クイズエンジン・RPG戦闘・ガチャ・コスチューム・ステータス・記録・UI・キャラ/音声/ユーザー管理 | 上記すべて |
| 17 | `cloud-sync.js` (defer) | 同期 | Firestore 同期・セッションロック・自己修復 | localStorage キー規約に依存 |
...
- **バトルアニメーション（2026-07-14）**：RPGバトルの攻撃/被弾/勝利/やられは
  ...
  root位置/回転・`c3dBody`/`c3dHead`/`c3dWingN`（`_c3dCollectParts` が命名）をトラック対象にする。
  モンスター12プランは体型で4カテゴリ（limbed/winged/amorphous/rooted）に分類し
  （`_C3D_MON_CAT`）、カテゴリ共通のクリップを使い回す。`index.html` のバトル処理からは
  `_c3dTriggerCombat(ラッパー要素, kind)` だけを呼ぶ（mixer/クリップの内部構造は非公開）。
  再生中は `_c3dTick` の手続きアイドル（揺れ・呼吸・首かしげ）を `v.animState` で止めて
  ミキサーに姿勢の所有権を渡す（まばたきだけは常時継続）。`prefers-reduced-motion` 時は
  再生しない。テストは `tests/unit-char3d-anim.js`（カテゴリ網羅・トラック名解決・
  mixer.update の補間値まで Node で実証）。
