...
| 14 | `index.html` inline `<script>` | 結合コア | クイズエンジン・RPG戦闘・ガチャ・コスチューム・ステータス・記録・UI・キャラ/音声/ユーザー管理 | 上記すべて |
| 15 | `cloud-sync.js` (defer) | 同期 | Firestore 同期・セッションロック・自己修復 | localStorage キー規約に依存 |
