...
c.eq('base種 74（＋Batch3の12体）', Object.keys(M.SRPG_MON_BASE_NAMES).length, 74);
c.eq('属性変種 240（48種×5・魔神/魔王は除外）', Object.keys(M.SRPG_MON_VARIANTS2).length, 240);
const dexAll = Object.keys(M.SRPG_MON_BASE_NAMES).concat(Object.keys(M.SRPG_MON_VARIANTS2));
c.eq('dex総数=314（＋Batch3の12体とその変種）', dexAll.length, 314);
