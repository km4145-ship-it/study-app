...
c.eq('base種 106（＋Batch6の8体）', Object.keys(M.SRPG_MON_BASE_NAMES).length, 106);
c.eq('属性変種 400（80種×5・魔神/魔王は除外）', Object.keys(M.SRPG_MON_VARIANTS2).length, 400);
const dexAll = Object.keys(M.SRPG_MON_BASE_NAMES).concat(Object.keys(M.SRPG_MON_VARIANTS2));
c.eq('dex総数=506（Batch6で500種到達）', dexAll.length, 506);
